## Using the solar regression data
...
if str(sys.argv[3]) == 'T':
	df = float(sys.argv[10])          # number of degrees of freedom for T dist.
...
if str(sys.argv[3]) == 'laplace':
	...
	W_1 = Laplace(loc=tf.zeros([n_hidden, K]), scale=std**2*(n_hidden**-1)*tf.ones([n_hidden, K]))
...
if str(sys.argv[3]) == 'T':
	...
	W_1 = StudentT(df=df*tf.ones([n_hidden, K]), loc=tf.zeros([n_hidden, K]), scale=abs(df/(df-2))*n_hidden**(-.5)*tf.ones([n_hidden, K]))
	b_0 = StudentT(df=df*tf.ones([n_hidden]), loc=tf.zeros(n_hidden), scale=tf.ones(n_hidden))
	b_1 = StudentT(df=df*tf.ones([K]), loc=tf.zeros(K), scale=abs(df/(df-2))*n_hidden**(-.5)*tf.ones(K))
...
if str(sys.argv[2]) == 'kl':
	if str(sys.argv[3]) == 'laplace':
		...
		qW_1 = Laplace(loc=tf.Variable(tf.random_normal([n_hidden, K])), 
		...
		qb_1 = Laplace(loc=tf.Variable(tf.random_normal([K])), 
	...
	if str(sys.argv[3]) == 'T':
		qW_0 = StudentT(df=df*tf.ones([D, n_hidden]), loc=tf.Variable(tf.random_normal([D, n_hidden])), 
			scale=tf.nn.softplus(tf.Variable(tf.random_normal([D, n_hidden]))))
		qW_1 = StudentT(df=df*tf.ones([n_hidden, K]), loc=tf.Variable(tf.random_normal([n_hidden, K])), 
			scale=tf.nn.softplus(tf.Variable(tf.random_normal([n_hidden, K]))))
		qb_0 = StudentT(df=df*tf.ones([n_hidden]), loc=tf.Variable(tf.random_normal([n_hidden])), 
			scale=tf.nn.softplus(tf.Variable(tf.random_normal([n_hidden]))))
		qb_1 = StudentT(df=df*tf.ones([K]), loc=tf.Variable(tf.random_normal([K])), 
			scale=tf.nn.softplus(tf.Variable(tf.random_normal([K]))))
...
if str(sys.argv[2]) != 'kl':
	...
	if str(sys.argv[3]) == 'laplace' or str(sys.argv[3]) == 'T':
		...
		p1 = tf.placeholder(tf.float32, [n_samp, n_hidden, K])
		pp0 = tf.placeholder(tf.float32, [n_samp, n_hidden])
		pp1 = tf.placeholder(tf.float32, [n_samp, K])
	...
	if str(sys.argv[2]) != 'kl':
		if str(sys.argv[3]) == 'laplace':
			sess.run(init, feed_dict={p0: np.random.laplace(size=[n_samp, D, n_hidden]),
					p1: np.random.laplace(size=[n_samp, n_hidden, K], scale = std**2*(n_hidden**-1)), 
					pp0: np.random.laplace(size=[n_samp, n_hidden]),
					pp1: np.random.laplace(size=[n_samp, K], scale = std**2*(n_hidden**-1))})
		if str(sys.argv[3]) == 'T':
			sess.run(init, feed_dict={p0: np.random.standard_t(df, size=[n_samp, D, n_hidden]),
					p1: np.random.standard_t(df, size=[n_samp, n_hidden, K]), 
					pp0: np.random.standard_t(df, size=[n_samp, n_hidden]),
					pp1: np.random.standard_t(df, size=[n_samp, K])})

if str(sys.argv[3]) == 'normal' or str(sys.argv[2]) == 'kl':
	tf.global_variables_initializer().run()

if str(sys.argv[3]) != 'T':
	path =  ('../saved/' + str(n_hidden) +'units/1l_' + str(inference.n_iter) + 'rep/' + 
...
for _ in range(inference.n_iter):
	...
	if (_ + 1 ) % 50 == 0 or _ == 0:
		...
		acc_tmp = ed.evaluate('mse', data={x: X_test, y_post: Y_test}, n_samples=500)
...
if str(sys.argv[2]) == 'kl':
	...
	W0_opt = qW_0.sample(mini_samp).eval().mean(axis=0)
	W1_opt = qW_1.sample(mini_samp).eval().mean(axis=0)
	b0_opt = qb_0.sample(mini_samp).eval().mean(axis=0)
	b1_opt = qb_1.sample(mini_samp).eval().mean(axis=0)
	...
	file_name = path + '/mse_500samples_from_posterior.npy'
	np.save(file_name, mse_list)

	file_name = path + '/predictions_500samples_from_posterior.npy'
	np.save(file_name, preds)
...
# x is the vector we want to plot, e.g. qW_0[:, i, j]
...
x_in = np.linspace(-2, 2, 200)
samples = 30
W0_samp, W1_samp, b0_samp, b1_samp = (qW_0.sample(samples).eval(), qW_1.sample(samples).eval(), 
	qb_0.sample(samples).eval(), qb_1.sample(samples).eval())
