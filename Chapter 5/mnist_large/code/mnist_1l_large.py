...
def ed_graph_init():
	...
	if str(sys.argv[4]) == 'T':
		W_0 = StudentT(df=df*tf.ones([D, n_hidden]), loc=tf.zeros([D, n_hidden]), scale=std2*2/D*tf.ones([D, n_hidden]))
...
with tf.Session() as sess:
	...
	acf_vals = np.zeros([24, n_lags_used])
	...
	trace_plotw = np.zeros([12, n_samp])
	...
	np.save(path + '/traceplot_w0.npy', np.reshape(trace_plotw, [-1, 12, n_samp]))
	...
	for i in range(3):
		for j in range(4):	
			...
			trace_plotw[i*3+j, :] = w_samp
	np.save(path + '/traceplot_w1.npy', np.reshape(trace_plotw, [-1, 12, n_samp]))
...
for phase_iter in range(1, n_iter_learn):
	...
	# Build predictive graph
	x_pred, ww0, ww1, bb0, bb1, y_pred = pred_graph()
	# Build probabilities predictive graph
	...
	with tf.Session() as sess:
		...
		np.save(path + '/traceplot_w0.npy', 
			np.concatenate([tmp_tr, np.reshape(trace_plotw, [-1, 12, n_samp])], 0))
...
for i in range(3):
	for j in range(4):
		...
		sns.distplot(qW1_smp[:, ii1[j], jj1[i]], hist=False, rug=False, ax=ax[1])
...
with tf.Session() as sess:
	...
	# ax[0].imshow((conf_mat_mean.T/np.sum(conf_mat_mean, axis=1)).T, cmap=plt.cm.gnuplot)
