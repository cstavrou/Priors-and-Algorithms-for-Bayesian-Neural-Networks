...
def ed_graph_init():
	...
	if str(sys.argv[4]) == 'laplace' or str(sys.argv[4]) == 'T':
		return ((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, 
...
if str(sys.argv[4]) == 'laplace' or str(sys.argv[4]) == 'T':
	((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, 
		qW_3, qb_3, inference, p0, p1, p2, p3, pp0, pp1, pp2, pp3, w0, w1, w2, w3, b0, b1, b2, b3) = ed_graph_init()
else:
	((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, 
		qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, qW_3, qb_3, inference) = ed_graph_init()
...
for phase_iter in range(1, n_iter_learn):
	...
	if phase_iter == 1 and str(sys.argv[4]) == 'normal':
		del ((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, 
		qW_3, qb_3, inference)
	else:
		del ((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, 
		qW_3, qb_3, inference, p0, p1, p2, p3, pp0, pp1, pp2, pp3, w0, w1, w2, w3, b0, b1, b2, b3)
	...
	if phase_iter > 1:
		del x_in, w0_fin, b0_fin, w1_fin, b1_fin, w2_fin, b2_fin, w3_fin, b3_fin, prob_out
	...
	# Build the initial graph inference graph
	((x, y), y_ph, W_0, b_0, W_1, b_1, W_2, b_2, W_3, b_3, 
		qW_0, qb_0, qW_1, qb_1, qW_2, qb_2, 
		qW_3, qb_3, inference, p0, p1, p2, p3, 
		pp0, pp1, pp2, pp3, w0, w1, w2, w3, b0, b1, b2, b3) = ed_graph_2(inc_step)
	...
	with tf.Session() as sess:
		...
		for j in range(0, n_samp, 10):
			prob = sess.run(prob_out, feed_dict={x_in: X_train[trace_ind, :], w0_fin: qW_0.params.eval()[j, :, :, :, :], 
				b0_fin: qb_0.params.eval()[j, :], w1_fin: qW_1.params.eval()[j, :, :, :, :], 
				b1_fin: qb_1.params.eval()[j, :], w2_fin: qW_2.params.eval()[j, :, :], b2_fin: qb_2.params.eval()[j, :], 
				w3_fin: qW_3.params.eval()[j, :, :], b3_fin: qb_3.params.eval()[j, :]})
		...
		if phase_iter == 1:
			...
			tmp = qW_3.params.eval()[nburn:n_samp:n_eff, :, :]
			np.save(path + '/qW3_samp.npy', tmp)
			...
			tmp = qb_3.params.eval()[nburn:n_samp:n_eff, :]
			np.save(path + '/qb3_samp.npy', tmp)
			...
			tmp = qW_3.params.eval()[nburn:n_samp:100, :, :]
			np.save(path + '/qW3_samp_n100.npy', tmp)
			...
			tmp = qb_3.params.eval()[nburn:n_samp:100, :]
			np.save(path + '/qb3_samp_n100.npy', tmp)
			...
			tmp = np.load(path + '/qW3_samp.npy')
			np.save(path + '/qW3_samp.npy', np.concatenate([tmp, qW_3.params.eval()[nburn:n_samp:n_eff, :, :]], 0))
			...
			tmp = np.load(path + '/qb3_samp.npy')
			np.save(path + '/qb3_samp.npy', np.concatenate([tmp, qb_3.params.eval()[nburn:n_samp:n_eff, :]], 0))
			...
			tmp = np.load(path + '/qW3_samp_n100.npy')
			np.save(path + '/qW3_samp_n100.npy', np.concatenate([tmp, qW_3.params.eval()[nburn:n_samp:100, :, :]], 0))
			...
			tmp = np.load(path + '/qb3_samp_n100.npy')
			np.save(path + '/qb3_samp_n100.npy', np.concatenate([tmp, qb_3.params.eval()[nburn:n_samp:100, :]], 0))			
		...
		qW3_smp = np.load(path + '/qW3_samp.npy')
...
qb3_smp = np.load(path + '/qb3_samp.npy')
